...
class ARM2RV64Translator:
    ...
    def _generate_baremetal(self):
        ...
        ws = self.data_width // 8  # word size in bytes
        funct3_s = 0x3 if self.data_width==64 else 0x2
        funct3_l = 0x3 if self.data_width==64 else 0x2
        store_name = "sd" if self.data_width==64 else "sw"
        load_name = "ld" if self.data_width==64 else "lw"
        shift = 3 if self.data_width==64 else 2

        # Emit: addi x19, x0, data_addr  (array base)
        ...
        for i,val in enumerate(array_data):
            ...
            self.emit_li(11, vs)
            off = i * ws
            self.emit(f"{store_name} x11, {off}(x19)",
                      encode_s(off&0xFFF,11,19,funct3_s), f"arr[{i}] = {vs}")
        ...
        for idx, inst in enumerate(arm_code):
            ...
            # Skip: sub sp, sp, #56 (stack allocation)
            if op=='sub' and len(ops)>=3:
                try:
                    ...
                    if (parse_arm_register(ops[0])=='sp' and
                        ...
                        skip_indices.add(idx); continue
                    # sub ip, fp, #56 (array copy dest setup)
